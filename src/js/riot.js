...
(function (global, factory) {
  ...
  var tmpl = (function () {
    ...
    function _wrapExpr (expr, asText, key) {
      ...
      if (key) {
        ...
        expr = `function(v){${tb ?
          expr.replace("return ", "v=") : `v=(${expr})`};return v||v===0?v:""}.call(this)`
      }
  ...
  function updateExpression (expr) {
    var dom = expr.dom,
      ...
      value = isToggle ? tmpl(expr.expr, extend({}, this, this.parent)) :
      tmpl(expr.expr, this),
    ...
    if (isFunction(value)) {
      ...
      // <img src="{ expr }">
    } else if (startsWith(attrName, RIOT_PREFIX) && attrName !==
      RIOT_TAG_IS) {
      ...
      if (attrName === "selected" && parent && (/^(SELECT|OPTGROUP)$/).test(
  ...
  var IfExpr = {
    ...
    "update": function update () {
      ...
      if (newValue && !this.current) { // insert
        ...
        this.expressions = []
        parseExpressions.apply(this.parentTag, [this.current, this.expressions,
          true])
  ...
  var RefExpr = {
    ...
    "update": function update () {
      ...
      var customParent = this.parent && getImmediateCustomParentTag(
        this.parent)
  ...
  function _each (dom, parent, expr) {
    ...
    var mustReorder = typeof getAttr(dom, "no-reorder") !== T_STRING ||
    ...
    expr.update = function updateEach () {
      ...
      if (ifExpr) {
        items = items.filter(function (item, i) {
          if (expr.key) {
            return Boolean(tmpl(ifExpr, mkitem(expr, item, i, parent)))
          }

          // in case it's not a keyed loop
          // we test the validity of the if expression against
          // the item and the parent
          return Boolean(tmpl(ifExpr, parent)) || Boolean(tmpl(ifExpr, item))
        })
      }
      ...
      each(items, function (item, i) {
        ...
        item = !hasKeys && expr.key ? mkitem(expr, item, i) :
          item
  ...
  function parseExpressions (root, expressions, mustIncludeRoot) {
    ...
    walkNodes(root, function (dom, ctx) {
      ...
      if (attr = getAttr(dom, "if")) {
        parent.children.push(Object.create(IfExpr)
          .init(dom, this$1, attr))
        return false
      }
      ...
      if (tagImpl && (dom !== root || mustIncludeRoot)) {
        ...
        parent.children.push(initChildTag(tagImpl, conf, dom.innerHTML,
          this$1))
        return false
      }

      // attribute expressions
      parseAttributes.apply(this$1, [dom, dom.attributes, function (
        attr, expr) {
        if (!expr) {
          return
        }
  ...
  function parseAttributes (dom, attrs, fn) {
    ...
    each(attrs, function (attr) {
      ...
      if (~["ref", "data-ref"].indexOf(name)) {
        expr = Object.create(RefExpr)
          .init(dom, name, attr.value, this$1)
  ...
  */
  ...
  var reYieldSrc =
    /<yield\s+to=['"]([^'">]*)['"]\s*>([\S\s]*?)<\/yield\s*>/ig
  var reYieldDest =
    /<yield\s+from=['"]?([-\w]+)['"]?\s*(?:\/>|>([\S\s]*?)<\/yield\s*>)/ig
  var rootEls = {
    "tr": "tbody",
    "th": "tr",
    "td": "tr",
  ...
  var tblTags = IE_VERSION && IE_VERSION < 10 ? RE_SPECIAL_TAGS :
  ...
  */
  ...
  function tag2$$1 (name, tmpl, css, attrs, fn) {
    ...
    var exists = Boolean(__TAG_IMPL[name])
  ...
  function mount$$1 (selector, tagName, opts) {
    ...
    function pushTagsTo (root) {
      if (root.tagName) {
        ...
        var tag$$1 = mountTo(root, riotTag || root.tagName.toLowerCase(),
          opts)
  ...
  function updateOpts (isLoop, parent, isAnonymous, opts, instAttrs) {
    ...
    each(instAttrs, function (attr) {
      ...
      opts[toCamel(attr.name)] = attr.expr ? attr.expr.value : attr
        .value
    })
  }
  ...
  function Tag$$1 (impl, conf, innerHTML) {
    ...
    defineProperty(this, "update", function tagUpdate (data) {
      ...
      extend(this, data)
      updateOpts.apply(this, [isLoop, parent, isAnonymous, opts,
        instAttrs])
      if (this.isMounted) {
        ...
        each(props, function (key) {
          ...
          if (key !== "init") {
            ...
            var descriptor = Object.getOwnPropertyDescriptor(
              instance, key) || Object.getOwnPropertyDescriptor(
              proto, key)
            var hasGetterSetter = descriptor && (descriptor
              .get || descriptor.set)
    ...
    defineProperty(this, "mount", function tagMount () {
      ...
      parseAttributes.apply(parent, [root, root.attributes,
        function (attr, expr) {
      ...
      parseAttributes.apply(this, [root, implAttrs, function (attr,
        expr) {
        if (expr) {
      ...
      if (this._parent && isAnonymous) {
        inheritFrom.apply(this, [this._parent,
          propsInSyncWithParent])
      }
          ...
          if (isVirtual) {
            Object.keys(this.tags)
              .forEach(function (tagName) {
                arrayishRemove(ptag.tags, tagName, this$1.tags[
                  tagName])
              })
          } else {
  ...
  function inheritFrom (target, propsInSyncWithParent) {
    ...
    each(Object.keys(target), function (k) {
      ...
      var mustSync = !isReservedName(k) && contains(
        propsInSyncWithParent, k)
  ...
  function cleanUpData (data) {
    if (!(data instanceof Tag$$1) && !(data && typeof data.trigger ===
        T_FUNCTION)) {
      return data
    }
